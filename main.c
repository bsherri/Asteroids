...
int
update_field (int field[][FIELD_WIDTH], int ship_row)
{
...
if (dice < P_LARGER_ASTEROID)
	    field[i][j] = ASTEROID_VAL;
...
void
update_screen (int field[][FIELD_WIDTH])
{
...
if (update_field (asteroid_field, ship_row))
	break;
...
mvwaddch (stdscr, 0, 0, '-');
	...
	mvwaddch (stdscr, 0, 0, '0');
      
if (ship_row < 0)
	ship_row = 0;
      
if (ship_row >= FIELD_HEIGHT)
	ship_row = FIELD_HEIGHT - 1;
